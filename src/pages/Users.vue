<template>
   <div class="row">
        <div class="col-12">
            <button class="add-button" @click="toggleModal()">new user</button>
            <card :title="title">                
                            ...
                            <tr v-for="item in users" :key="item.id">
                                <slot>
                                    ...
                                    <td>
                                        ...
                                        <a href="" class="nav-link" @click.prevent="deleteUser(item)"> <i class="tim-icons icon-trash-simple"></i></a>
                                    </td>
                                </slot>
                            </tr>
                        </tbody>
        ...
        <transition name="modal" v-if="showModal">
            ...
            <div class="modal-container">
                ...
                <div class="modal-footer">
                    <slot name="footer">
                        <button class="modal-default-button" @click="isUpdate == true ? editUser() : addUser()">
                            save
                        </button>
...
export default {
    data() {
        return {
            title: "Users list",
            tableColumns: ["id", "Name", "email","username","actions"],
            users: [],
            form : {
                id : '',
                name : '',
    ...
    methods : {
        ...mapActions({
            ...
            destroyeUser: 'users/destroyeUser',
            updateUser: 'users/updateUser',
        }),
        applyUsers(){
            this.fetchusers()
            .then(()=> this.users=this.getUsers)
        },
        initialize(){
            this.applyUsers()
        },
        ...
        deleteUser(user){
            Swal.fire({
                title: 'Delete user',
                text: `Are you sure yo want to delete this user :  ${user.name}`,
                icon: 'warning',
        ...
        editUser(){
            ...
            .then(()=>{
                ...
                Swal.fire(
                        'updated',
                        `${this.form.name} infos has been updated.`,
                        'success'
                    )
                this.cancelHandle()
            })
            .catch(()=>{
                Swal.fire(
                        'oops!',
                        `${this.form.name} infos has not been updated.`,
                        'warning'
                    )
            })
        }
